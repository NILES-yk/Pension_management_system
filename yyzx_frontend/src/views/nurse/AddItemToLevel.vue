  ...
  methods: {
    ...
    getNurseItemList() {
      findNurseItemPage(this.queryParams).then(res => {
        ...
        this.page.total = res.data.data.total; //总记录数
        this.page.pageSize = res.data.data.size; //每页显示条数
        this.page.currentPag = res.data.data.current; //当前页码
        this.page.pagCount = res.data.data.pages; //总页数
      });
    },
    //api-查询当前级别护理项目
    queryNurseItemByLevel() {
      queryNurseItemByLevel({ levelId: this.levelId }).then(res => {
