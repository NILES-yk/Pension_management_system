...
import { queryNurseLevel, queryNurseItemByLevel } from "@/api/nurseLevelApi.js";
import { addCustomerNurseItem,removeCustomerNurseLevelAndItem} from "@/api/customerNurseItemApi.js";
  ...
  methods: {
    ...
    isOk() {
      ...
      for(let i=0;i<customerItemList.length;i++){
        ...
        obj.customerId=this.drawer.customerId
    ...
    listCustomerInfoPage() {
      // 添加时间戳避免缓存
      this.condition._t = new Date().getTime();
    ...
    //api-查询护理级别列表
    queryNurseLevel() {
      queryNurseLevel({ levelStatus: 1 }).then(res => {
        this.drawer.listLevel = res.data;
      });
    },
    //api-查询当前级别护理项目
    queryNurseItemByLevel() {
      queryNurseItemByLevel({ levelId: this.drawer.levelId }).then(res => {
