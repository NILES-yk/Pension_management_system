...
export default {
  ...
  methods: {
    ...
    getNurseItemList() {
      ...
      const params = {
        ...this.queryParams,
        pageSize: parseInt(this.queryParams.pageSize) || 6, // 默认6条
        currentPage: this.page.currentPage // 添加当前页码参数
    };

    findNurseItemPage(params).then(res => {
        console.log("完整响应:", res);
        if (res && res.data && res.data.data) {
          this.nurseItemList = res.data.data.records || [];
          console.log("表格数据:", this.nurseItemList);
          
          // 从正确的路径获取分页数据
          if (res.data.data.total !== undefined) {
            this.page.total = parseInt(res.data.data.total);
          }
          if (res.data.data.size !== undefined) {
            this.page.pageSize = parseInt(res.data.data.size);
          }
          if (res.data.data.current !== undefined) {
            this.page.currentPage = parseInt(res.data.data.current);
          }
          if (res.data.data.pages !== undefined) {
            this.page.pagCount = parseInt(res.data.data.pages);
          }
        } else {
    ...
    handleSizeChange(size) {
      ...
      this.queryParams.pageSize = size.toString(); // 保持与后端一致
      this.getNurseItemList();
    }
  }
};
