...
<script>
import { findNurseItemPage } from "@/api/nurseContentApi.js";
import { isIncludesCustomerNurseItem ,addCustomerNurseItem} from "@/api/customerNurseItemApi.js";
...
export default {
  ...
  mounted() {
    this.getNurseItemList();
     // 如果有标签页组件
      if (window.parent.updateTabTitle) {
        window.parent.updateTabTitle('购买护理服务');
      }
  },
  computed: {
    indexMethod() {
  ...
  methods: {
    ...
    changeRow(row) {
        ...
        isIncludesCustomerNurseItem({customerId:this.customerId,itemId:row.id}).then(res=>{
            if(!res.flag){
            ...
            let obj={
              ...row,
              buyTime:formatDate(today),
              maturityTime:formatDate(threeMonthsLater),
              nurseNumber:1
            };
            //判断是否已经添加过
            ...
            //     return
            // }
            if(this.changeNurseItemList.some(item=>item.id===obj.id)){
              this.$message.warning("所选项目已存在列表中")
              return
            }
    ...
    //api-查询护理项目(分页)
      ...
      findNurseItemPage(this.queryParams).then(res => {
        console.log("完整护理项目响应:", JSON.stringify(res, null, 2)); // 添加这行
        ...
        if (res && res.data) {
          ...
          if (Array.isArray(records)) {
            this.nurseItemList = records.map(item => ({
              ...item,
              // 确保字段名与表格列prop匹配
              serialNumber: item.serial_number || item.serialNumber,
              nursingName: item.nursing_name || item.nursingName,
              servicePrice: item.service_price || item.servicePrice,
              executionCycle: item.execution_cycle || item.executionCycle,
              executionTimes: item.execution_times || item.executionTimes,
    ...
    isOk() {
      ...
      this.$confirm("确定添护理项目数据无误?", "提示", {
        ...
        .then(() => {
          addCustomerNurseItem(paramsArr).then(res=>{
              if (res.flag) {
