...
<script>
import { listCustomerInfoPage,updateCustomer } from "@/api/customerApi.js";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    getNotManagerKhxxList() {
      listCustomerInfoPage(this.queryParams).then(res => {
    ...
    //api-查询当前管家护理客户
    getManagerKhxxList() {
      listCustomerInfoPage(this.manager.queryParams).then(res => {
    ...
    del(id) {
      this.$confirm("确定移除?", "提示", {
        ...
        .then(() => {
         updateCustomer({ id: id, userId: -1 }).then(res => {
            if (res.flag) {
              //刷新表格
              this.getManagerKhxxList();
              this.getNotManagerKhxxList();
