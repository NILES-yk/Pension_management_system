        ...
        <el-row :gutter="20">
          ...
          <el-col :span="14">
            <div class="table-main">
              ...
              <el-table :data="customerItemList" size="small">
                ...
                <el-table-column align="center" prop label="状态">
                  <template #default="scope">
                    ...
                    <el-tag
                      v-if="getDaysDiff(scope.row.maturityTime)>10"
                      type="success"
                      disable-transitions
                    >未到期</el-tag>
                    <el-tag
                      v-if="getDaysDiff(scope.row.maturityTime)>0 && getDaysDiff(scope.row.maturityTime)<=10"
                      type="warning"
                      disable-transitions
                    >即将到期</el-tag>
                    <el-tag
                      v-if="getDaysDiff(scope.row.maturityTime)<=0"
                      type="danger"
                      disable-transitions
                    >已到期</el-tag>
                  </template>
              ...
              <!-- 分页插件 -->
              <div style="margin-top:15px" v-if="customerItemList&&customerItemList.length>0">
                <el-pagination
...
export default {
  ...
  methods: {
    getDaysDiff(maturityTime){
      if(!maturityTime) return 0
      const maturityDate = parseDate(maturityTime)
      const today=new Date()
      return Math.floor((maturityDate-today)/(1000*60*60*24))
    },
    //点击查询
    query() {
      //清空护理项目数据表
    ...
    buyService() {
      ...
      '$route': {
        handler(newVal) {
          if (newVal.path === '/health/serviceBuy') {
            // 动态设置标签页标题
            this.$store.commit('updateTabTitle', {
              path: newVal.path,
              title: '购买护理服务'
            });
          }
    ...
    //api-选中某客户行:获取用户的服务项目列表
        queryCustomerNurseItem() {
      console.log("请求参数:", this.conditionItem);
      queryCustomerNurseItem(this.conditionItem).then(res => {
        console.log("完整API响应结构:", JSON.stringify(res, null, 2));
        
        // 修改后的数据处理逻辑
        if (res && res.data && res.data.data) {
          const responseData = res.data.data; // 获取实际数据层级
          
          // 处理记录数据
          this.customerItemList = Array.isArray(responseData.records) 
            ? responseData.records.map(item => ({
                ...item,
                // 确保字段名与表格列prop匹配
                ...
                nurseNumber: item.nurseNumber || 0,
                maturityTime: item.maturityTime || ''
              }))
            : [];
          
          // 更新分页信息（从responseData获取）
          this.pageItem = {
            total: responseData.total || 0,
            pageSize: responseData.size || 6,
            currentPag: responseData.current || 1,
            pagCount: responseData.pages || 0
          };
        } else {
          this.customerItemList = [];
    ...
    del(id) {
      this.$confirm("此操作将移除当前护理项目, 请谨慎操作?", "提示", {
        ...
        .then(() => {
          removeCustomerNurseLevelAndItem({ id: id }).then(res => {
            if (res.flag) {
              this.$message({
                type: "success",
                message: res.message
    ...
    save(formName) {
      this.$refs[formName].validate(valid => {
        ...
        if (valid) {
          customerRenewal(params).then(res => {
            if (res.flag) {
