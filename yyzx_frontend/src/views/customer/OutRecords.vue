...
import {queryOutwardPage,addOutward,examineOutward, removeOutward,updateBackTime} from "@/api/outwardApi.js";
import {getSessionStorage,getCurrentDate} from '@/utils/common.js'
...
export default {
  ...
  methods: {
    ...
    //api-查询外出申请记录
    queryOutwardPage(){
      queryOutwardPage(this.conditionRecord).then(res=>{
	...
	examineOutward(formName){
		this.$refs[formName].validate(valid => {
		  if (valid) {
			...
			this.dialog.item.audittime = getCurrentDate();
	  ...
	  this.$refs[formName].validate(valid => {
	    if (valid) {
	      ...
	      if (this.dialog.item.id == null || this.dialog.item.id == "") {
			this.dialog.item.createTime = getCurrentDate();
